...
function sphere(block, radius) {
    ...
    // mid section
    //
    this.up(radius - slices[0][1]).cylinder(block, radius, (slices[0][1] * 2) - 1, {
        blockType: bm[0],
        meta: bm[1]
    ...
    for(i = 1; i < slices.length; i++) {
        ...
        h = radius - sr;
        // northern hemisphere
        this.up(v).fwd(h).right(h).cylinder(block, sr, sh, {
            blockType: bm[0],
            meta: bm[1]
...
function hemisphere(block, radius, northSouth) {
    ...
    //
    if(northSouth == 'north') {
        this.cylinder(block, radius, slices[0][1], {
            blockType: bm[0],
            meta: bm[1]
        });
    } else {
        this.up(radius - slices[0][1]).cylinder(block, radius, slices[0][1], {
            blockType: bm[0],
            meta: bm[1]
        }).down(radius - slices[0][1]);
    }
