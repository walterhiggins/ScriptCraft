...
function prism(block, w, d) {
    ...
    if(stairEquiv) {
        this.fwd().prism(stairEquiv, w, d - 2).back().stairs(block, w, d / 2).fwd(d - 1).right(w - 1).turn(2).stairs(block, w, d / 2).turn(2).left(w - 1).back(d - 1);
    } else {
        var c = 0;
        var d2 = d;
        while(d2 >= 1) {
...
function prism0(block, w, d) {
    this.stairs(block, w, d / 2).fwd(d - 1).right(w - 1).turn(2).stairs(block, w, d / 2).turn(2).left(w - 1).back(d - 1);
    var se = STAIRBLOCKS[block];
    if(se) {
        this.fwd().prism(se, 1, d - 2).right(w - 1).prism(se, 1, d - 2).left(w - 1).back();
    }
}
