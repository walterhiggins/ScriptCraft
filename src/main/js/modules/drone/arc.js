...
function getStrokeDir(x, y) {
    ...
    var absX = Math.abs(x);
    var strokeDir = 0;
    if(y > 0 && absY >= absX) strokeDir = 0; //down
    else if(y < 0 && absY >= absX) strokeDir = 1; // up
    else if(x > 0 && absX >= absY) strokeDir = 2; // left
    else if(x < 0 && absX >= absY) strokeDir = 3; // right
    return strokeDir;
};
/*
    ...
    if(orientation == 'horizontal') {
        ...
        setPixel = function(x, y) {
            ...
            if(params.fill) {
                ...
                if(y < 0) {
                    drone.fwd(y).right(x).cuboidX(params.blockType, params.meta, 1, stack, Math.abs(y * 2) + 1).back(y).left(x);
                }
            } else {
                if(strokeWidth == 1) {
                    gotoxy(x, y).cuboidX(params.blockType, params.meta, 1, // width
                        stack, // height
                        strokeWidth // depth
                    ).move('center');
                } else {
                    ...
                    gotoxy(x, y).cuboidX(params.blockType, params.meta, width, stack, depth).move('center');
                }
            }
        };
    } else {
        ...
        setPixel = function(x, y) {
            ...
            if(params.fill) {
                // wph 20130114 more efficient esp. for large cylinders/spheres
                if(y < 0) {
                    drone.up(y).right(x).cuboidX(params.blockType, params.meta, 1, Math.abs(y * 2) + 1, stack).down(y).left(x);
                }
            } else {
                if(strokeWidth == 1) {
                    gotoxy(x, y).cuboidX(params.blockType, params.meta, strokeWidth, 1, stack).move('center');
                } else {
                    var strokeDir = getStrokeDir(x, y);
                    var width = 1,
                        height = 1;
                    ...
                    gotoxy(x, y).cuboidX(params.blockType, params.meta, width, height, stack).move('center');
                }
            }
        };
    }
