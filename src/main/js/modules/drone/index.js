...
Drones can be created in any of the following ways...
 ...
 2. Using `self`, which is the current player object, as the source of location details...
 ...
 5. Create a new Drone based on another Drone...

    var myClone = new Drone(existingDrone);

For more complex building projects where more than one drone is desired, this is a convenience option which copies the x,y,z,dir,world of the existing drone into a new object. Note that this is not the same as:

    var second = first;

... where the second variable is now just a reference to the first, rather than a separate object.
...
function isTooBig(w, h, d) {
    return(w * h * d) >= Drone.MAX_VOLUME || (w >= Drone.MAX_SIDE) || (h >= Drone.MAX_SIDE) || (d >= Drone.MAX_SIDE);
};
/*
...
Drone.prototype.cuboidX = function(blockType, meta, w, h, d, immediate) {
    ...
    if(isTooBig(w, h, d)) {
        this.sign(['Build too Big!', 'width:' + w, 'height:' + h, 'depth:' + d], 68);
...
Drone.prototype.cuboid0 = function(block, w, h, d, immediate) {
    ...
    this.chkpt(start).cuboid(block, w, h, 1, immediate) // Front wall
        .cuboid(block, 1, h, d, immediate) // Left wall
        .right(w - 1).cuboid(block, 1, h, d, immediate) // Right wall
        .left(w - 1).fwd(d - 1).cuboid(block, w, h, 1, immediate) // Back wall
        .move(start);
};
...
var droneCoreExts = ['./arc', './bed', './blocktype', './copypaste', './cylinders', './doors', './firework', './garden', './ladder', './movement', './prism', './rand', './sign', './sphere', './stairs', './trees'];
