...
function getPlayerPos(player) {
    ...
    if(player) {
        ...
        if(__plugin.canary) {
            if(player instanceof Packages.net.canarymod.api.world.blocks.CommandBlock) return player.block.location;
            else return player.location;
        }
    }
    return null;
...
exports.serverAddress = function serverAddress() {
    ...
    while(interfaces.hasMoreElements()) {
        ...
        while(addresses.hasMoreElements()) {
            ...
            if(current_addr instanceof java.net.Inet4Address) return current_addr.getHostAddress();
        }
    }
    return null;
};
