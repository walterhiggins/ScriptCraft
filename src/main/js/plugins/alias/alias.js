...
var _global = function(params, player) {
    if(!isOp(player)) {
        echo(player, 'Only operators can set global aliases. ' + 'You need to be an operator to perform this command.');
        return;
    }
    var o = _processParams(params);
    ...
    try {
        if(_store.players[player.name]) {
            ...
            for(alias in _store.players[player.name]) {
                echo(player, alias + ' = ' + JSON.stringify(_store.players[player.name][alias]));
            }
        } else {
