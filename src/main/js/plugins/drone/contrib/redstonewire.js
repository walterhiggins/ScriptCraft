...
Drone.extend('wireblock', function(blockType) {
    this.chkpt('wireblock').box(blockType, 1, 2, 1) // 2 blocks tall, top block will be wire dropped on lower
        .up();
...
Drone.extend('repeaterblock', function(blockType) {
    this.chkpt('repeaterblock').box(blockType, 1, 2, 1).up();
...
Drone.extend('redstoneroad', function(roadBlockType, redstoneunderBlockType, distance) {
    return this.down().wirestraight(redstoneunderBlockType, distance).right().box(roadBlockType, 4, 1, distance).right(4).wirestraight(redstoneunderBlockType, distance).up();
});