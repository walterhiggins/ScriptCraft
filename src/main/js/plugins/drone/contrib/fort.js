...
function fort(side, height) {
    ...
    // build walls.
    //
    this.chkpt('fort').down().chessboard(blocks.wool.black, blocks.wool.white, side).up().box0(blocks.brick.stone, side, height - 1, side).up(height - 1);
    //
    // build battlements
    //
    for(i = 0; i <= 3; i++) {
        ...
        this.box(blocks.brick.stone) // solid brick corners
            .up().box(blocks.torch).down() // light a torch on each corner
            .fwd().boxa(turret, 1, 1, side - 2).fwd(side - 2).turn();
    }
    //
    // build battlement's floor
    //
    this.move('fort').up(height - 2).fwd().right();
    for(i = 0; i < battlementWidth; i++) {
        var bside = side - (2 + (i * 2));
        this.box0(blocks.slab.oak, bside, 1, bside).fwd().right();
    }
    //
    // add door
    //
    ...
    //
    this.move('fort').right((side / 2) - 3).fwd() // move inside fort
        .turn(2).box(blocks.air, 1, height - 1, 1).ladder(height - 1).move('fort');
}
Drone.extend(fort);